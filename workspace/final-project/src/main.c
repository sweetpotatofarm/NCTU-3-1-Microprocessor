	...
	while(1){
		...
		if(i == 1){
			...
			if( value >= 5000 && value <= 85000){
				...
				//two more times
				for(int i = 0 ; i < 2 ; i++){
					if( HX711_value() >= 5000 && HX711_value() <= 85000 )
						check = 0;
					else{
						check = 1;
						break;
				...
				//put back
				if(difference > 25000 && value >= 100000){
					flag = 1;
					//time to drink water
					if(last_weight - value < 4000 && last_empty == 0){
						alarm_handler();
					}
					//last time is empty, now add water
					if(last_weight - value < 4000 && last_empty == 1){
						last_empty = 0;
					}
					n = 0;
					SetSysTickValue();
